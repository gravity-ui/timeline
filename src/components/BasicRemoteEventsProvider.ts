...
type Loader<T extends TimelineEvent> = (
...
export class BasicRemoteEventsProvider<
...
> extends TimelineComponent {
  ...
  protected handleTimeRangeChanged = debounce_(
    (newRange: ContinuousRange): void => {
  ...
  public hostConnected() {
    ...
    this.handleTimeRangeChanged(
      new ContinuousRange(this.host.start, this.host.end),
    );
  }
...
class BasicTimelineEventsLoader<
  T extends TimelineEvent,
> extends AbstractTimelineDataLoader<EventsRegion<T>> {
  constructor(
    options: TimelineRegionsManagerOptions<EventsRegion<T>> & {
      loadWindow: Loader<T>;
    },
  ) {
    super(options);
